...
const MAX_ARRAY_LENGTH: u32 = 5000;
...
pub unsafe fn find_methods_in_assembly_by_param(
    ...
    log!(
        "Scanning {} classes for methods taking '{}'...",
        class_count,
        param_type_name
    );
    ...
    for i in 0..class_count {
        ...
        if let Some(method) = find_method_taking_param_by_name(klass, param_type_name) {
            ...
            let method_name = if method_name_ptr.is_null() {
                ...
                CStr::from_ptr(method_name_ptr).to_string_lossy().to_string()
            };
...
// ObscuredInt / ObscuredLong / ObscuredFloat XOR decryption
...
pub unsafe fn convert_object_to_value(
    ...
    if class_name.ends_with("[]") {
        ...
        if class_name == "Single[]" || class_name == "System.Single[]" {
            ...
            let res = Value::Array(
                slice
                    .iter()
                    .map(|v| Number::from_f64(*v as f64).map(Value::Number).unwrap_or(Value::Null))
                    .collect(),
            );
            visited.remove(&obj_addr);
            return res;
        ...
        if !element_class.is_null() {
            ...
            if !is_value_type {
                ...
                for i in 0..len {
                    let elem_ptr = *((data_start as *mut *mut RawIl2CppObject).add(i as usize));
                    if !elem_ptr.is_null() {
                ...
                if stride > 0 {
                    ...
                    for i in 0..len {
                        ...
                        let val = if FN_CLASS_IS_ENUM.unwrap()(element_class) {
                            ...
                            let name = CStr::from_ptr(name_ptr).to_string_lossy();
                            if name.starts_with("Obscured") {
                                try_decrypt_obscured(elem_ptr as *mut c_void, element_class, &name)
                                    .unwrap_or_else(|| {
    ...
    while !current_class.is_null() {
        let mut iter: *mut c_void = ptr::null_mut();
        loop {
            let field = FN_CLASS_GET_FIELDS.unwrap()(current_class, &mut iter);
            if field.is_null() {
                break;
            }
            ...
            if !field_filter.is_empty() && !field_filter.contains(&name.as_str()) {
                continue;
            }
        ...
        current_class = FN_CLASS_GET_PARENT.unwrap()(current_class);
        if !current_class.is_null() {
            let p_name = FN_CLASS_GET_NAME.unwrap()(current_class);
