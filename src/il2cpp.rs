...
pub unsafe fn find_image_by_name(name: &str) -> *mut RawIl2CppImage {
    let domain = match FN_DOMAIN_GET {
        Some(f) => f(),
    ...
    let assemblies = match FN_DOMAIN_GET_ASSEMBLIES {
        Some(f) => f(domain, &mut count),
        None => return ptr::null_mut(),
    };
    ...
    for i in 0..count {
        ...
        let image = match FN_ASSEMBLY_GET_IMAGE {
            Some(f) => f(assembly),
            None => return ptr::null_mut(),
        };
        if image.is_null() {
        ...
        let image_name_ptr = match FN_IMAGE_GET_NAME {
            Some(f) => f(image),
            None => return ptr::null_mut(),
        };
...
pub unsafe fn get_class_from_image(
    ...
    match FN_CLASS_FROM_NAME {
        Some(f) => f(image, namespace, class_name),
        None => ptr::null_mut(),
    }
}
...
pub unsafe fn find_method_addr_by_name(
    ...
    let method = match FN_CLASS_GET_METHOD_FROM_NAME {
        Some(f) => f(klass, method_name, arg_count),
        None => return 0,
    };
    method_addr(method)
}
